...
const getDeleteKeyframe = (state) => state.deleteKeyframe;
...
const stateFactory = (set, get) => ({
    ...
    deleteKeyframe: ({ elementName, valueId, id }) => {
        ...
        set({
            animations: produce(animations, (draft) => {
                const elementValues = draft[selectedAnimationName].elements[elementName];
                const valueAnimation = elementValues.find((value) => value.id === valueId);
                if (!valueAnimation)
                    return;
                ...
                const isFinalKeyframe = finalKeyframe.id === id;
                console.log(orderedKeyframes.map((keyframe) => keyframe.offset));
                // Rescale animation offsets if we're removing the final keyframe
                if (isFinalKeyframe) {
                    orderedKeyframes.pop();
                    const { duration = defaults.duration } = valueAnimation.options;
                    let newDuration = duration;
                    const newFinalKeyframe = orderedKeyframes[orderedKeyframes.length - 1];
                    if (!newFinalKeyframe) {
                        newDuration = 0;
                    }
                    else {
                        newDuration = newFinalKeyframe.offset * duration;
                        for (const keyframe of orderedKeyframes) {
                            const time = keyframe.offset * duration;
                            valueAnimation.keyframes[keyframe.id].offset = time / newDuration;
                        }
                    }
...
const useEditorState = create(stateFactory);
...
function TrashIcon({ style }) {
    return (react.exports.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 512 512", style: style },
        react.exports.createElement("title", null, "Trash"),
        react.exports.createElement("path", { d: "M112 112l20 320c.95 18.49 14.4 32 32 32h184c17.67 0 30.87-13.51 32-32l20-320", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "32" }),
        react.exports.createElement("path", { stroke: "currentColor", strokeLinecap: "round", strokeMiterlimit: "10", strokeWidth: "32", d: "M80 112h352" }),
        react.exports.createElement("path", { d: "M192 112V72h0a23.93 23.93 0 0124-24h80a23.93 23.93 0 0124 24h0v40M256 176v224M184 176l8 224M328 176l-8 224", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "32" })));
}
...
function ValueControl({ keyframeMetadata, valueAnimation }) {
    ...
    return (react.exports.createElement(ActionsContainer, null,
        react.exports.createElement(DeleteButton, { onClick: () => deleteKeyframe(keyframeMetadata) },
            react.exports.createElement(TrashIcon, { style: {
                    width: 16,
                    height: 16,
                    color: "var(--red)",
                    marginRight: 5,
